            ...
            <div class="row justify-content-center">
                <div class="col-md-8">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('mentor.quizzes.update', $quiz) }}" method="POST">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror"
                                           value="{{ old('title', $quiz->title) }}" required maxlength="255">
                                    @error('title')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" id="description" rows="4" class="form-control @error('description') is-invalid @enderror">{{ old('description', $quiz->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" name="time_limit" id="time_limit" class="form-control @error('time_limit') is-invalid @enderror"
                                                   value="{{ old('time_limit', $quiz->time_limit) }}" min="1" placeholder="Tidak terbatas">
                                            @error('time_limit')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                ...
                                <div class="mb-3">
                                    <div class="form-check">
                                        ...
                                        <label class="form-check-label" for="is_active">
                                            📢 Quiz Aktif (Siswa dapat mengakses)
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('mentor.quizzes.show', $quiz) }}" class="btn btn-secondary">Batal</a>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const currentMaterialId = {{ $quiz->material_id ?? 'null' }};

            courseSelect.addEventListener('change', function() {
                const courseId = this.value;
                const currentSelected = materialSelect.value;
                    ...
                    fetch(`/mentor/courses/${courseId}/materials`)
                        ...
                        .then(materials => {
                            materials.forEach(material => {
                                ...
                                if (material.id == currentMaterialId || material.id == currentSelected) {
